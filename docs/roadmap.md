...
- [ ] 建立不依赖模型生图能力的本地确定性图片来源，作为图片能力的基线和降级路径：
      复用 Playwright 录制器已产出的预览帧、项目登记的产品截图作为文章配图和视频
      封面的真实素材，并提供本地文本叠加/封面合成（标题、活动主题、渠道规格尺寸
      的纯文本版式），让没有图像生成模型的 MCP 宿主也能产出可用的图片产物。
