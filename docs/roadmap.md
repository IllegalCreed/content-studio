...
SQLite 可以创建或替换 `(projectId, channelId)` 的唯一绑定；工作台渠道管理页可以保存
项目启停状态、脱敏账号别名、`channelAccountRef` 和交付方式。活动仍然只选择项目已启用
的渠道，账号由项目绑定解析。该操作只写 Content Studio 本地配置，不读取凭据、不配置
`marketing-ops` 授权，也不执行真实发布。全局渠道定义和 `marketing-ops` 新鲜状态仍待
后续客户端接入。
