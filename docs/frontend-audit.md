...
- **已完成第二轮**：活动、任务、报告和素材的原始数据转展示数据逻辑已集中到
  `projections.ts` 的 `activityToCampaign()`、`taskToProjection()`、`runtimeReports()`、
  `runtimeActivityArtifacts()` 和 `runtimeProjectAssets()`；工作台只负责提供运行时数据
  和项目账号别名，页面组件不再自己理解 core 类型。
...
2. **状态集中化（第二轮完成）**：选择状态、页面筛选、运行时连接状态和共享业务快照已
   ...
   页面继续只负责展示和事件派发。
3. **投影模块化（已完成）**：活动、任务、报告和素材的展示投影已从工作台编排层或通用
   模型拆到 `projections.ts`，并有单元测试；渠道投影仍按同样方式逐步收拢。
