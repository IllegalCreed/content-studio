...
Content Studio 自己登记的文件，并提供预览、确认、回收区和恢复窗口。当前 Runtime 已
提供只读占用统计和清理预览；确认、回收区和恢复窗口仍未实现。V0.3.0 的内存 Repository
仍用于纯内存测试；`SqliteContentStudioRepository` 已提供第一版本地控制数据持久化。
...
方形 1080×1080；当前不允许工作台或 MCP 任意修改宽高。
...
项目素材和活动产物；尺寸、时长、清理确认、回收区和恢复仍由后续存储切片补齐。
