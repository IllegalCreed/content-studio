...
2. 写入前获取该账号和渠道的最新健康、策略与授权状态；只读状态客户端先校验当前
   ...
   安装器/宿主通过显式 `MarketingOpsManagedRuntime` 注入已初始化 client 和关闭句柄，
   CLI 只在命令结束时幂等关闭，不自行探测或启动进程；
...
成片阶段由 FFmpeg 合成器消费已登记的片段，并把目标 `outputSize`、短转场和响度策略
落实到最终资源变体。默认生产合成随后从成片首帧生成本地确定性封面：PNG 帧以内嵌
`data:` 形式保存于 `cover.svg`，标题和渠道/语言副标题以固定文本版式叠加，返回尺寸、
大小和 sha256，并登记为同一活动下的 `image` artifact。这样没有图像生成模型的 MCP
宿主仍可得到可预览的封面；可选 provider 只能在同一活动产物、任务取消和校验和契约之上
扩展，不能把模型凭据带入 MCP 或本地文件。
同一成片的次级动图预览使用本地 FFmpeg 的 `palettegen`/`paletteuse` 两遍流程生成
`preview.gif`，默认取 4 秒、10 FPS、640px 长边并以 `-loop 0` 无限循环；编码过程不含
音轨，受帧数、像素、时长和文件大小上限约束，完成后同样登记为活动级 `image` artifact。
Workbench 和 Runtime 沿用现有图片预览契约，通过 `.gif` 扩展名提供 `image/gif` 响应，
不会新增第二套媒体或发布引擎。

合成执行沿用同一任务的 `composing` 阶段，而不是建立第二套队列。开始合成、成片完成、
封面完成、GIF 完成以及终态都会追加结构化任务事件；成功回执按任务尝试记录活动产物 ID、
安全相对路径、尺寸、时长、帧率、大小和 sha256，取消/失败回执只记录失败原因，不把
不稳定的半成品路径暴露为可用产物。回执与事件由任务存储写入 SQLite，Runtime 项目视图、
MCP `tasks/get` 和 Workbench 读取同一份状态。`AbortSignal` 会继续传入 FFmpeg 子进程、
媒体探测、文件哈希、封面和 GIF 生成；取消后清理临时清单，且应用服务不会登记半成品。
...
发布活动先保存主题、目标和目标渠道，再按渠道规格规划内容。文章、图文、动态和视频
是渠道成品类型；同一渠道可以在一次活动中选择多个形态。图片、封面、字幕、配音、背景
音乐、脚本、视频片段和预览帧属于活动素材或中间产物。渠道成品引用这些素材并携带标题、
正文、媒体、资源变体和渠道元数据；只有逐形态媒体 readiness 通过后才能进入发布安排。

`ChannelBlueprint.contentForms` 是逐形态能力的唯一扩展点：每个形态声明标题/正文长度、
允许的媒体类型和最小/最大媒体数量，并以第一个形态标记渠道默认值。旧的
`format`/`supportedFormats` 字段继续保留用于兼容旧 campaign；生成器和新的活动表单以
逐形态蓝图为准。应用服务只把渠道内容显式引用的最终 `image`/`video` 活动产物计入
readiness；`preview-frame`、`video-clip` 和其他中间产物不计入。视频合成成功后，成片、
封面和 GIF 资源变体会以新内容版本回写到对应渠道内容，其中最终成片满足视频形态要求。
