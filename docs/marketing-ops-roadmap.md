...
启动前还会再次检查已安装目录的 owner、私有权限、固定文件清单、软链接和硬链接；这只是
本地安装完整性复查，不能替代正式发行签名。当前摘要仍由安装器调用方提供，仓库没有官方公钥、
私钥或下载/公证链，因此正式的安装器和发行流程仍未接入；测试公钥不会被当成生产信任根。

严格的 Ed25519 发行声明解析与平台选择现已作为安装器专用的纯内存验签器落地：它要求
`keyId` 与调用方提供的公钥表精确绑定，拒绝非 canonical JSON、未知字段、错误签名、兼容性
不符和非 `darwin/arm64` 目标，并返回冻结的数据对象。这里的公钥表仍只是调用方输入，尚未
指定官方公钥，也不会自动下载、安装或启动 runtime；下一步仍需正式安装器提供内置/已验证的
信任根、归档校验与回执重验。
...
未带引用的响应才临时回退到 alias。工作台只会把仍新鲜的当前已解析账号作为一个本项目
候选项，必须由项目所有者明确保存本地绑定，绝不自动改绑。它仍不提供全局账号目录、多账号
选择、发布包账号字段或写入权。
