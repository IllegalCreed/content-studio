...
- 渠道平台能力由全局管理，账号在项目渠道配置中绑定且每个渠道唯一；发布活动只选择
  渠道，账号由项目配置自动解析。
- 渠道是内容形态能力集合，而不是单一格式。例如 Bilibili 可以在同一个活动中选择
  `short-post`（动态）、`image-text`（图文）和 `video-metadata`（视频稿）；每种形态都会
  生成独立内容包、文件和后续制作任务。视频的横屏/竖屏不是另一个渠道，而是活动视频
  计划的 `format: "landscape" | "portrait"`，并会被锁进 Bilibili 发布包和人工确认回执。
  活动创建时通过 `contentFormats` 选择要做的形态；未声明时仍使用渠道默认形态，保持旧
  campaign 兼容。当前一个活动的视频计划只有一个画幅；若同一主题要同时出横屏和竖屏，
  应先建立两条独立的视频发布安排，后续再把画幅下沉为逐内容变体。
- 每个渠道形态蓝图还描述标题/正文长度、允许的媒体类型和媒体数量要求。应用服务会把
  渠道内容引用的最终 `image`/`video` 活动产物投影为逐内容 readiness，MCP 和工作台会
  显示“已就绪”或缺少的资源；预览帧和录制片段不能冒充最终成品。缺少必需媒体时不会
  创建发布安排。这个本地门槛不等同于发布授权，也不会替代独立 `marketing-ops` 的最终校验。
- 活动产物可标记 `en`、`zh-CN` 或语言无关的 `neutral`；渠道内容只能引用同语言或
  neutral 产物。发布包以渠道、语言、内容形态和内容版本锁定，只携带 artifact ID、sha256
  和版本，不携带本地路径；同一渠道的中英文或多形态包不会被互相覆盖。发布安排建立后，
  `prepare_marketing_ops_package` 会从项目内自动解析当前渠道绑定并只读生成该包；调用方不能
  传入账号引用或路径，返回结果也不会产生渠道调用、授权或发布回执。
- `marketing-ops` 状态只通过有类型的只读客户端读取；当前兼容矩阵为 Content Studio
  ...
  对已解析账号，快照可带不透明 `accountRef`；工作台优先据此匹配项目绑定，引用不匹配时
  保持阻塞，旧响应缺失该字段才兼容回退到脱敏 alias。快照仍新鲜时，工作台只把当前
  已解析账号作为该项目的单个候选项；项目所有者明确保存后才写入本地绑定，不自动改绑、
  不形成全局账号目录，也不授予发布权限。
  安装器或宿主通过显式的 `MarketingOpsManagedRuntime` 注入已初始化 client 和关闭句柄；
  CLI 只负责消费并在命令结束时幂等关闭，不探测命令、路径或凭据。
  Plugin 通过独立的 `content-studio-host` 入口启动，它只转发三个项目路径变量；在安装器
  交付有签名或内置摘要的固定 runtime 制品前，host 不会尝试查找或启动任何 `marketing-ops`
  进程，状态保持阻塞。
...
活动详情还可以通过本地应用服务保存内容组和渠道内容版本，按渠道展示文章、图文、动态或视频内容。
已有内容可以用带 `baseVersion` 的媒体修订追加或替换最终图片/视频；替换只更新最终媒体引用，
保留文章、音频和中间证据，建立发布安排后则锁定媒体修订。当前工作台里的保存表单是
AI/MCP 接入前的手动测试入口，不代表系统已经自动生成内容。

视频制作任务在录制片段完成后会由本地 FFmpeg 合成为渠道尺寸的最终成片，并从成片首帧
生成一个嵌入真实画面的 `composed/cover.svg`。封面以活动级图片产物登记，工作台和本地
Runtime 预览接口可以直接查看；这条降级路径不调用图像模型，也不接收模型凭据。
同一成片还可以生成一个无音频、无限循环的 `composed/preview.gif`：默认限制为 640px
长边、10 FPS、4 秒和 8 MiB，并以同一活动级 `image` 产物登记，方便公众号等场景直接
预览或下载。GIF 使用本地 FFmpeg 的调色板两遍编码，不调用图像模型，也不授予任何发布权限。
合成阶段现在沿用任务事件和回执契约：视频、封面、GIF 完成时分别写入结构化事件，终态
回执按任务尝试保存安全相对路径、尺寸、时长、大小和 SHA-256。取消信号会传入 FFmpeg、
媒体探测和文件哈希；取消或失败的尝试不会把半成品登记为活动产物，回执可在 SQLite 重启后恢复。
...
工具不会执行真实渠道发布。图片等最终产物稍后生成时，可用
`revise_channel_content_media` 对已有内容执行版本化 `append`/`replace`，无需重建正文。
...
当前视频录制使用横屏、竖屏、方形三种固定分辨率预设，也支持在项目和活动视频计划中
指定经过边长、像素总量、比例和画幅方向校验的 viewport、输出媒体尺寸、device scale
factor、语言和色彩环境。配置按项目默认值、活动默认值和渠道变体合并，最终完整配置会
进入 `VideoPlan` 和录制回执；不接受任意浏览器参数、脚本、
选择器、cookie 或凭据。
工作台的新建活动表单可以建立视频制作计划，活动详情可以修订录制尺寸；每次修订都会
生成新版本并等待再次确认。
...
统一素材预览首版已接入：项目素材和活动产物会通过本地运行时的项目范围接口显示文字、
图片、视频或音频预览，并提供登记文件下载；运行时未连接时仍只显示登记信息。尺寸、时长、
放大查看、清理确认、回收区和恢复按路线图继续补齐；素材页已可查看已登记文件的只读
清理预览，不会扫描或删除未知文件。
