...
watch(() => route.path, (path) => {
  activeModule.value = moduleForPath(path)
})
          ...
          <article class="module-card">
            ...
            <div class="module-list">
              <button
                ...
                @click="openActivityDetail(campaign.campaignId)"
              >
          ...
          <div class="project-overview-grid project-overview-grid-lower">
            ...
            <article class="module-card">
              ...
              <div class="module-list compact-list">
                <button v-for="campaign in snapshot.campaigns" :key="campaign.campaignId" type="button" @click="openActivityDetail(campaign.campaignId)">
                  <span class="list-status">{{ humanizeActivityStatus(campaign.activityStatus) }} · {{ activityTaskSummary(campaign.campaignId) }}</span>
                  <strong>{{ campaign.title }}</strong>
            ...
            <article class="campaign-detail">
              <div class="detail-heading">
                ...
                <div class="detail-heading-actions">
                  ...
                  <button type="button" class="primary-button" data-testid="activity-detail-link" @click="openActivityDetail(selectedCampaign.campaignId)">打开活动详情</button>
                </div>
              </div>
        ...
        <section id="reports" class="module-section">
          ...
          <div v-if="snapshot.reports.length > 0" class="report-list">
            <article v-for="report in snapshot.reports" :key="`${report.activityId}-${report.channel}`" class="report-card">
              ...
              <button type="button" class="report-link" @click="openActivityDetail(report.activityId)">查看所属活动 →</button>
            </article>
          </div>
