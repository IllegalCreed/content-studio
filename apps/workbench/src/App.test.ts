...
describe('content studio workbench', () => {
  ...
  it('把规划中的模块作为可切换的功能页面展示', async () => {
    ...
    expect(wrapper.findAll('.task-detail .task-step-list > li')).toHaveLength(5)
    ...
    await wrapper.get('button[data-channel-id="bilibili"]').trigger('click')
    expect(wrapper.get('[data-testid="channel-form-specs"]').text()).toContain('至少 1 张图片')
    expect(wrapper.get('[data-testid="channel-form-specs"]').text()).toContain('需要 1 个视频')
    await wrapper.get('button[data-channel-id="github"]').trigger('click')
